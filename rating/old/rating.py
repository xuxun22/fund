...
#benchmark_data = pd.read_csv("http://ichart.yahoo.com/table.csv?s=%s&a=%s&b=%s&c=%s&d=%s&e=%s&f=%s" % (benchmark_symbol['symbol'],start_date.month - 1, start_date.day, start_date.year,end_date.month - 1, end_date.day, end_date.year),parse_dates = ["Date"])
